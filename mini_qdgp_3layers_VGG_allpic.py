...
q_weights = torch.nn.Parameter(torch.randn(n_layers, n_qubits, 2) * 0.5) # 稍微加大初始抖动
optimizer = torch.optim.Adam([{"params": gen.parameters()}, {"params": q_weights}], lr=0.005)

print(f"🚀 启动 64x64 感知驱动修复 (遮罩范围: {mask_width} 像素)...")
for step in range(801): # 像素变大，步数也增加一点
    optimizer.zero_grad()
    z = torch.stack(q_circuit(q_weights)).float().unsqueeze(0).to(device)
    generated_img = gen(z)
    
    pixel_loss = torch.mean(((generated_img - target) * mask)**2)
    vgg_loss = 1.5 * get_perceptual_loss(generated_img, target) # 提高 VGG 权重以增强细节
...
print(f"🎉 高清修复任务完成！文件夹: {output_dir}")